...
describe('This is first test suite, Ekaterina Seliste', () => {
    it('User can submit data only when valid mandatory values are added', () => {
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('#firstName').type('Martin')
        cy.get('[data-testid="lastNameTestId"]').type('Trevor')        
        cy.get('input[name="password"]').type('Ekaterina1234')
        cy.get('[name="confirm"]').type('Ekaterina1234')
    ...
    */
    ...
    it('User cannot add letters to phone number', () => {
        cy.get('#username').type('johnDoe')
        cy.get('[data-testid="phoneNumberTestId"]').type('Cerebrum Hub', { force: true })
        cy.get("input[name='password']").type('Password123')
        cy.get('[name="confirm"]').type('Password123')
    ...
    cy.get('[data-testid="phoneNumberTestId"]').scrollIntoView()
        ...
        // Attempt to fill in the phone number with letters
        cy.get('[data-testid="phoneNumberTestId"]').type('Cerebrum Hub', { force: true })
